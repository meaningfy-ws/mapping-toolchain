...
from mapping_toolchain.mapping_suite_processor.entrypoints.cli.cmd_sparql_queries_injector import main as cli_main


def test_sparql_queries_injector(cli_runner, fake_mapping_suite_id, file_system_repository_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        ...
        response = cli_runner.invoke(cli_main, [fake_mapping_suite_id, "--opt-mappings-folder",
                                                temp_mapping_suite_path])
        assert response.exit_code == 0
        assert "SUCCESS" in response.output


def test_sparql_queries_injector_with_non_existing_input(cli_runner, file_system_repository_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_suite_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_suite_path, dirs_exist_ok=True)
