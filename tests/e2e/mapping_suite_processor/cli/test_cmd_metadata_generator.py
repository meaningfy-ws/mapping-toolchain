...
from mapping_toolchain.mapping_suite_processor.entrypoints.cli.cmd_metadata_generator import main as cli_main
...
def test_metadata_generator_with_non_existing_input(cli_runner, file_system_repository_path, fake_test_mapping_suite_id):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_suite_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_suite_path, dirs_exist_ok=True)

        response = cli_runner.invoke(cli_main, [fake_test_mapping_suite_id,
...
def test_metadata_generator_with_invalid_input(cli_runner, file_system_repository_path, fake_test_mapping_suite_id):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_suite_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_suite_path, dirs_exist_ok=True)

        response = cli_runner.invoke(cli_main, [fake_test_mapping_suite_id,
                                                "-i", str(temp_mapping_suite_path / fake_test_mapping_suite_id /
